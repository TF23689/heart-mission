...
(function () {
  ...
  // --------- Sparkles (A: subtle) ----------
  // Small amount of particles, soft motion, pink/gold mix.
  ...
  function sparkleBurst(anchorEl) {
    ...
    function draw(t) {
      ...
      for (const s of particles) {
        // ease out
        const k = 1 - p;
        const px = s.x + s.vx * (p * 70);
        const py = s.y + s.vy * (p * 70) + p * p * 22;

        const alpha = Math.max(0, (k * 0.9));
        ctx.globalAlpha = alpha;

        // draw tiny sparkle (diamond)
  ...
  // --------- Day page logic ----------
  // Expect <body data-page="day" data-day="0922">
  async function initDayPage() {
    ...
    // Fill header text if placeholders exist
    ...
    function setLockedUI(isUnlocked) {
      ...
      if (statusBadge) {
        statusBadge.textContent = isUnlocked ? data.uiText.labels.unlocked : data.uiText.labels.locked;
      }
    }

    // Unlock
    const isUnlocked = unlocked.has(dayId);
    setLockedUI(isUnlocked);
    ...
    // If day unlocked initially, but panel hidden, render is still ok.
    // Hide missions if locked
    if (!unlocked.has(dayId) && unlockedPanel) unlockedPanel.style.display = "none";
  }
  // --------- Missions page logic ----------
  // Expect <body data-page="missions">
